...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 	{
		...
		while (queue.size() > 0) {
			...
			WPTreeNode holderNode = queue.remove(0);
			
			List<String> list = nw.distanceOne(holderNode.getWord(),true);
			
			for(String distanceOneWord : list) {
				
				if (!visited.contains(distanceOneWord)) {
					
					WPTreeNode distanceOneWordNode = new WPTreeNode(distanceOneWord,holderNode);
					queue.add(distanceOneWordNode);
			...
			if (queue.size() > 0) {
				holderNode = queue.get(queue.size()-1);
			}
			
			if (holderNode.getWord().equals(word2)) {
				rootNode = holderNode;
				break;
			}
		...
		if (rootNode.getWord() != word1) {
			return rootNode.buildPathToRoot();
		} else {
			return null;
		}
	}
